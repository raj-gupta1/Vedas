"""
Sushruta Samhita (English Translation) Scraper
Scrapes all 6 volumes from wisdomlib.org chapter by chapter.
Outputs JSONL for continuous pretraining.

Usage:
    python sushruta_scraper.py [--delay 2] [--output ../continuousPreTrainStyle/data/sushruta_samhita_pretrain.jsonl] [--limit 2]
"""
...
def extract_links_from_index(html):
    ...
    links = []
    # Match any sushruta samhita volume chapter doc URL
    pattern = r'<a\s+href="(/hinduism/book/sushruta-samhita-volume-\d+-[a-zA-Z0-9_-]+/d/doc\d+\.html)">(.*?)</a>'
...
def scrape_sushruta(output_file, delay=2.0, limit=None):
    os.makedirs(os.path.dirname(output_file) or ".", exist_ok=True)

    volumes = [
        ("Sushruta Samhita, volume 1: Sutrasthana", "https://www.wisdomlib.org/hinduism/book/sushruta-samhita-volume-1-sutrasthana"),
        ("Sushruta Samhita, volume 2: Nidanasthana", "https://www.wisdomlib.org/hinduism/book/sushruta-samhita-volume-2-nidanasthana"),
        ("Sushruta Samhita, volume 3: Sharirasthana", "https://www.wisdomlib.org/hinduism/book/sushruta-samhita-volume-3-sharirasthana"),
        ("Sushruta Samhita, volume 4: Cikitsasthana", "https://www.wisdomlib.org/hinduism/book/sushruta-samhita-volume-4-cikitsasthana"),
        ("Sushruta Samhita, volume 5: Kalpasthana", "https://www.wisdomlib.org/hinduism/book/sushruta-samhita-volume-5-kalpasthana"),
        ("Sushruta Samhita, volume 6: Uttara-tantra", "https://www.wisdomlib.org/hinduism/book/sushruta-samhita-volume-6-uttara-tantra")
    ]
        ...
        for vol_title, vol_url in volumes:
            ...
            for title, url in filtered_links:
                if limit and vol_count >= limit:
                    print(f"   Reached limit of {limit} chapters for this volume.")
                    break
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrape Sushruta Samhita from wisdomlib.org → JSONL")
    parser.add_argument("--delay", type=float, default=2.0,
                        help="Delay between requests in seconds (default: 2)")
    parser.add_argument("--output", type=str,
                        default="/Users/raj/PycharmProjects/VedaGPT/continuousPreTrainStyle/data/sushruta_samhita_pretrain.jsonl",
                        help="Output JSONL file path")
    parser.add_argument("--limit", type=int, default=None,
                        help="Limit number of chapters to scrape per volume (for testing)")
    args = parser.parse_args()

    scrape_sushruta(output_file=args.output, delay=args.delay, limit=args.limit)
