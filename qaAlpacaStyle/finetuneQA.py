...
class CustomProgressCallback(TrainerCallback):
    def on_log(self, args, state, control, logs=None, **kwargs):
        if state.max_steps is not None and state.max_steps > 0:
            ...
            print(f"\n📊 {Fore.LIGHTCYAN_EX}SFT Training Progress: {current_step}/{total_steps} steps ({percent:.2f}%) | Epoch: {epoch:.2f}{Fore.RESET}")
            if logs:
                if "loss" in logs:
...
# Format mapping function for chat template
...
def main():
    print(f"🚀 {Fore.LIGHTCYAN_EX}Starting SFT QA Fine-Tuning Setup...{Fore.RESET}")
    ...
    if not os.path.exists(train_file) or not os.path.exists(val_file):
        raise FileNotFoundError("QA split files not found! Make sure to run 'splitCleanedQAData.py' inside the qaAlpacaStyle directory first.")

    print(f"📂 Loading split datasets: {train_file} and {val_file}...")
    train_dataset = load_dataset("json", data_files=train_file)["train"].map(
        lambda x: format_chat_template(x, tokenizer), 
        num_proc=8, 
        batched=True, 
        batch_size=10
    )
    val_dataset = load_dataset("json", data_files=val_file)["train"].map(
        lambda x: format_chat_template(x, tokenizer), 
        num_proc=8, 
        batched=True, 
        batch_size=10
    ...
    # Save complete models (User Style)
    trainer.save_model('outputs/complete_checkpoint')
    trainer.model.save_pretrained("outputs/final_model")
    print("🎉 Done! Models saved to 'outputs/complete_checkpoint' and 'outputs/final_model'.")
